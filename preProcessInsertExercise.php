...
<body>
	...
	<section id="accountNav">
	    ...
	    $exerciseName = $conn->real_escape_string($_POST['exerciseName']);
	    $workoutType = $conn->real_escape_string($_POST['workoutType']);
	    $muscleGroup= $conn->real_escape_string($_POST['muscleGroup']);
	    $description= $conn->real_escape_string($_POST['description']);
	    $sets= $conn->real_escape_string($_POST['sets']);
	    $reps = $conn->real_escape_string($_POST['reps']);
	    ...
	    $_SESSION['workoutType'] = $workoutType;
	    $_SESSION['muscleGroup'] = $muscleGroup;
	...
	<a class='btn btn-success btn-lg' href='processInsertExercise.php' name = 'noButton' role='button'><font color = 'white'>Yes, this is the one</font></a>
	         </div>
	<div align = 'center' style = 'padding-top:10px'>
	<a class='btn btn-warning btn-lg' href='insertExercise.php' name = 'noButton' role='button'>No, this is wrong.</a>
	         </div> ";


	} else {
	header('Location: insertExerciseYoutubeError.php');
	}
	?><!--- Footer Section =-->
