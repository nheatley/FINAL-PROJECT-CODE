...
<body>
	...
	<form action='basicUsers.php' class='subform' id='firstNameForm' method='post' name="firstNameForm">
		<input class="btn btn-warning" id='defineSearch' name='defineSearch' type='submit' value='Search'> <input class='u-full-width' id='defineSearch' maxlength='50' name='defineSearch' placeholder="search..." type='text'> 
	           ...
	           echo "
	           <a class='btn btn-info' href='basicUsers.php' role='button'>All Users</a>
	           ";}
	           ?>
	<div style="padding-top:20px">
		<h2 align="center">Basic Users</h2>
	</div><?php 
	  ...
	  $result = $conn->query("SELECT * FROM TrainerPal_User WHERE userID = '$activeID' ");
	    ...
	    echo "did not work";
	     ...
	     $check = $conn->query("SELECT * FROM TrainerPal_User WHERE accountType = '1'");
	  ...
	  "
	      <div class = 'col-12' style ='border-bottom: gray 3px solid;'>
	        ...
	        <h5 align = 'center'> The total amount of basic users is: $num </h5>
	      </div>";         
	}
	?><?php
	...
	$resultB = $conn->query("SELECT * FROM TrainerPal_User WHERE accountType = '1'");
	 ...
	 <font color = 'red'> <h2 align='center'>There are no basic users at the moment.</h2></font>
	 </div>";
	 } 
	...
	<table class='table table-hover'>
	  ...
	  <tbody>
	    ...
	    $definedQuery = "SELECT TrainerPal_User.firstName, TrainerPal_User.lastName, TrainerPal_User.email, TrainerPal_User.telephoneNumber, TrainerPal_User.address,
	                            ...
	                            FROM TrainerPal_User WHERE TrainerPal_User.firstName LIKE '%$definedSearch%' AND TrainerPal_User.accountType ='1'
	                            OR (TrainerPal_User.lastName LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.email LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.telephoneNumber LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.address LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.city LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.postcode LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.accountType LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.country LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.accountType LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')
	                            OR (TrainerPal_User.userID LIKE '%$definedSearch%' AND TrainerPal_User.accountType = '1')";
