...
<body>
	...
	<section id="accountNav">
		<ul class="nav justify-content-center">
			...
			<li class="nav-item">
				<a class="nav-link" href="#"><font color="white">Sent Messages</font></a>
			</li>
			<li class="nav-item">
	...
	<div style="padding-top:20px; border-bottom:gray 3px solid;">
	  ...
	  $resultB = $conn->query("SELECT TrainerPal_User.firstName, TrainerPal_MessageHandler.messageID, TrainerPal_User.lastName, TrainerPal_User.profilePictureURL, TrainerPal_MessageHandler.recipientID,
	                           TrainerPal_MessageHandler.subject, TrainerPal_MessageHandler.messageID,
	                           TrainerPal_MessageHandler.message FROM TrainerPal_User INNER JOIN TrainerPal_MessageHandler ON TrainerPal_User.userID = TrainerPal_MessageHandler.recipientID 
	                           WHERE TrainerPal_MessageHandler.senderID = '$activeID' ORDER BY TrainerPal_MessageHandler.messageID DESC");
	        if(!$resultB) {
	    echo "did not work";
	    }
	...
	if ($num == 0) {  
	 echo " <font color = 'red'> <h2 align='center'>You have no sent messages!</h2></font>
	 ...
	 <a class='btn btn-primary' href='messengerHome.php' role='button'>Back to Messages</a>
	  ...
	  <img style = 'max-width: 150px' src='uploadedfiles/$profileImage' class='rounded-circle'>
	    <h3 class='mt-0'>You sent a message to $firstName $lastName</h3>
	    <h4>Subject: $subject</h4>
	    <p style = 'font-size:20px;'>$message</p>
	    <div align = 'center' style = 'padding-bottom:10px;'>
	     <a class='btn btn-primary' href='messengerHome.php' role='button'>Back to Messenger</a>
	     </div>
	  </div>
	</div>";    
